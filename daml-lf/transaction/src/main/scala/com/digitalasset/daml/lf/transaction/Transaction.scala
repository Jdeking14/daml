...
sealed abstract class HasTxNodes[Nid, +Cid, +Val] {
  ...
  final def inputContracts[Cid2 >: Cid]: Set[Cid2] =
    fold(Set.empty[Cid2]) {
      case (acc, (_, Node.NodeExercises(_, coid, _, _, _, _, _, _, _, _, _, _, _, _, _))) =>
        acc + coid
    ...
    tx.fold(State(Set.empty, Set.empty)) {
        case (state, (_, node)) =>
          node match {
            ...
            case Node.NodeExercises(_, _, tmplId, _, _, true, _, _, _, _, _, _, _, Some(key), _) =>
              state.consumed(globalKey(tmplId, key.key.value))
            case Node.NodeExercises(_, _, tmplId, _, _, false, _, _, _, _, _, _, _, Some(key), _) =>
